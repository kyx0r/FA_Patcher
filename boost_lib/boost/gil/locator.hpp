...
// typedefs required by ConstRandomAccess2DLocatorConcept
	...
	template <std::size_t D> typename axis<D>::iterator const& axis_iterator()                 const
	{
		return detail::locator_axis<D,Loc>()(concrete());
	}
	template <std::size_t D> typename axis<D>::iterator        axis_iterator(const point_t& p) const
	{
...
public:
	...
	{
		return memunit_step(y());    // distance in mem units (bytes or bits) between adjacent rows
	}
	std::ptrdiff_t         pixel_size()                         const
	{
		return memunit_step(x());    // distance in mem units (bytes or bits) between adjacent pixels on the same row
	}

	bool                   is_1d_traversable(x_coord_t width)   const
	{
		return row_size()-pixel_size()*width==0;    // is there no gap at the end of each row?
	}
