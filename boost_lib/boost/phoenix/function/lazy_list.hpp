...
public:
	...
	~strict_list()
	{
		while(rep && (rep->refC == 1)) rep = rep->tail;
	}
	...
	iterator begin() const
	{
		return impl::strict_list_iterator<T>( rep );
	}
	iterator end() const
	{
		return impl::strict_list_iterator<T>();
	}
...
public:
	...
	const force_result_type& force() const
	{
		return rep->cache();
	}
	const delay_result_type& delay() const
	{
		return *this;
	}
	// Note: force returns a reference;
	// implicit conversion now returns a copy.
	operator odd_list<T>() const
	{
		return force();
	}
...
public:
	...
	odd_list( const T& x, const list<T>& y ) : second(y)
	{
		init(x);
	}
	odd_list( const T& x, a_unique_type_for_nil ) : second(NIL)
	{
		init(x);
	}
	...
	iterator begin() const
	{
		return list_iterator<T>( this->delay() );
	}
	iterator end() const
	{
		return list_iterator<T>();
