...
shared_array() BOOST_SP_NOEXCEPT :
	px( 0 ), pn()
	{
	}
...
shared_array( boost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT :
	px( 0 ), pn()
	{
	}
...
shared_array( shared_array const & r ) BOOST_SP_NOEXCEPT :
	px( r.px ), pn( r.pn )
	{
	}

shared_array( shared_array && r ) BOOST_SP_NOEXCEPT :
	px( r.px ), pn()
	{
		pn.swap( r.pn );
		r.px = 0;
