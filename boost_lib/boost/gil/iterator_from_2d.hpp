...
public:
	...
	{
		return _width;    // number of pixels per image row
	}
	std::ptrdiff_t x_pos()         const
	{
		return _coords.x;    // current x position
	}
	std::ptrdiff_t y_pos()         const
	{
		return _coords.y;    // current y position
	}
	...
	{
		return _p.is_1d_traversable(width());    // is there no gap at the end of each row?
	}
	x_iterator&     x()
	{
		return _p.x();
