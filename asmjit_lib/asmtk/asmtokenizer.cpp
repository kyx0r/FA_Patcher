...
static inline uint32_t lower(uint32_t c)
{
	return c | 0x20;
}
...
uint32_t AsmTokenizer::next(AsmToken* token)
{
	...
	if (m == kCharPcn)
	{
		...
		{
		case '{':
			type = AsmToken::kLCurl   ;
			break;
		case '}':
			type = AsmToken::kRCurl   ;
			break;
		case '[':
			type = AsmToken::kLBracket;
			break;
		case ']':
			type = AsmToken::kRBracket;
			break;
		case '(':
			type = AsmToken::kLParen  ;
			break;
		case ')':
			type = AsmToken::kRParen  ;
			break;
		case '+':
			type = AsmToken::kAdd     ;
			break;
		...
		case '/':
			...
			break;
		case ',':
			type = AsmToken::kComma   ;
			break;
		case ':':
			type = AsmToken::kColon   ;
			break;
		}
		_cur = ++cur;
