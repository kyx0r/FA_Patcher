...
deque_back_view(Queue& q) BOOST_NOEXCEPT :
	queue(&q) {}
...
public:
	...
	// Constructors/Assignment/Destructors
deque_front_view(Queue& q) BOOST_NOEXCEPT :
	queue(&q) {}
	...
	queue_op_status nonblocking_push(const value_type& x)
	{
		return queue->nonblocking_push_front(x);
	}
	...
	queue_op_status wait_push(const value_type& x)
	{
		return queue->wait_push_front(x);
	}
	queue_op_status wait_pull(value_type& x)
	{
	...
	void push(BOOST_THREAD_RV_REF(value_type) x)
	{
		queue->push_front(forward<value_type>(x));
	}
	queue_op_status try_push(BOOST_THREAD_RV_REF(value_type) x)
	{
...
deque_back(deque_base<T>& q) BOOST_NOEXCEPT :
	base_type(q) {}
};
template <class T>
...
deque_front(deque_base<T>& q) BOOST_NOEXCEPT :
	base_type(q) {}
