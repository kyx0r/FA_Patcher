...
public:
	...
	ASMJIT_INLINE uint32_t getBlockSize() const noexcept
	{
		return _blockSize;
	}
	//! Get the default block alignment.
	ASMJIT_INLINE uint32_t getBlockAlignment() const noexcept
	{
		return (uint32_t)1 << _blockAlignmentShift;
	}
	//! Get remaining size of the current block.
	ASMJIT_INLINE size_t getRemainingSize() const noexcept
	{
		return (size_t)(_end - _ptr);
	}
	...
	ASMJIT_INLINE uint8_t* getCursor() noexcept
	{
		return _ptr;
	}
	//! Get the end of the current zone block, only useful if you use `getCursor()`.
	ASMJIT_INLINE uint8_t* getEnd() noexcept
	{
		return _end;
	}
...
class ZoneHeap
{
	...
	ASMJIT_INLINE void init(Zone* zone) noexcept
	{
		reset(zone);
	}
	...
	ASMJIT_INLINE Zone* getZone() const noexcept
	{
		return _zone;
	}
...
public:
	...
	{
		//! Get next node.
		ASMJIT_INLINE Link* getNext() const noexcept
		{
			return _next;
		}
		//! Get value.
		ASMJIT_INLINE T getValue() const noexcept
		{
			return _value;
		}
		//! Set value to `value`.
		ASMJIT_INLINE void setValue(const T& value) noexcept
		{
			_value = value;
		}
	...
	struct Appender
	{
		ASMJIT_INLINE Appender(ZoneList<T>& list) noexcept
		{
			init(list);
		}
	...
	ASMJIT_INLINE Link* getFirst() const noexcept
	{
		return _first;
	}
	ASMJIT_INLINE Link* getLast() const noexcept
	{
		return _last;
	}
...
public:
	...
	{
		_release(heap, sizeof(T));
	}

	//! Called to grow the buffer to fit at least `n` elements more.
	ASMJIT_INLINE Error grow(ZoneHeap* heap, size_t n) noexcept
	{
		return ZoneVectorBase::_grow(heap, sizeof(T), n);
	}
	...
	ASMJIT_INLINE Error resize(ZoneHeap* heap, size_t n) noexcept
	{
		return ZoneVectorBase::_resize(heap, sizeof(T), n);
	}

	//! Realloc internal array to fit at least `n` items.
	ASMJIT_INLINE Error reserve(ZoneHeap* heap, size_t n) noexcept
	{
		return ZoneVectorBase::_reserve(heap, sizeof(T), n);
	}
...
public:
	...
	//! Get data.
	ASMJIT_INLINE BitWord* getData() noexcept
	{
		return _data;
	}
	//! \overload
	ASMJIT_INLINE const BitWord* getData() const noexcept
	{
		return _data;
	}
...
public:
	...
	ASMJIT_INLINE ~ZoneHashBase() noexcept
	{
		reset(nullptr);
	}
...
public:
	...
	ASMJIT_INLINE Node* put(Node* node) noexcept
	{
		return static_cast<Node*>(_put(node));
	}
	ASMJIT_INLINE Node* del(Node* node) noexcept
	{
		return static_cast<Node*>(_del(node));
	}
};
