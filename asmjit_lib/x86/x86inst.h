...
struct X86Inst
{
	...
	struct CommonData
	{
		...
		ASMJIT_INLINE bool isUseR() const noexcept
		{
			return (getFlags() & kFlagUseX) == kFlagUseR;
		}
		//! Get if 1st operand is write-only.
		ASMJIT_INLINE bool isUseW() const noexcept
		{
			return (getFlags() & kFlagUseX) == kFlagUseW;
		}
		//! Get if 1st operand is read-write.
		ASMJIT_INLINE bool isUseX() const noexcept
		{
			return (getFlags() & kFlagUseX) == kFlagUseX;
		}
		//! Get if 1st and 2nd operands are read-write.
		ASMJIT_INLINE bool isUseXX() const noexcept
		{
			return hasFlag(kFlagUseXX);
		}

		ASMJIT_INLINE bool hasFixedReg() const noexcept
		{
			return hasFlag(kFlagFixedReg);
		}
		ASMJIT_INLINE bool hasFixedMem() const noexcept
		{
			return hasFlag(kFlagFixedMem);
		}
		ASMJIT_INLINE bool hasFixedRM() const noexcept
		{
			return hasFlag(kFlagFixedRM);
		}

		//! Get if the instruction is FPU instruction.
		ASMJIT_INLINE bool isFpu() const noexcept
		{
			return hasFlag(kFlagFpu);
		}
		//! Get if the instruction is MMX|3DNOW instruction that accesses MMX registers (includes EMMS).
		ASMJIT_INLINE bool isMmx() const noexcept
		{
			return hasFlag(kFlagMmx);
		}
		...
		{
			return hasFlag(kFlagVec);
		}
		//! Get if the instruction is SSE+ (SSE4.2, AES, SHA included) instruction that accesses XMM registers.
		ASMJIT_INLINE bool isSse() const noexcept
		{
			return (getFlags() & (kFlagVec | kFlagVex | kFlagEvex)) == kFlagVec;
		}
		//! Get if the instruction is AVX+ (FMA included) instruction that accesses XMM|YMM|ZMM registers.
		ASMJIT_INLINE bool isAvx() const noexcept
		{
			return isVec() && isVexOrEvex();
		}

		//! Get if the instruction can be prefixed by LOCK prefix.
		ASMJIT_INLINE bool isLockEnabled() const noexcept
		{
			return hasFlag(kFlagLock);
		}
		//! Get if the instruction can be prefixed by REP prefix.
		ASMJIT_INLINE bool isRepEnabled() const noexcept
		{
			return hasFlag(kFlagRep);
		}
		//! Get if the instruction can be prefixed by REPZ prefix.
		ASMJIT_INLINE bool isRepzEnabled() const noexcept
		{
			return hasFlag(kFlagRep);
		}
		//! Get if the instruction can be prefixed by REPNZ prefix.
		ASMJIT_INLINE bool isRepnzEnabled() const noexcept
		{
			return hasFlag(kFlagRepnz);
		}

		//! Get if the instruction uses MIB.
		ASMJIT_INLINE bool isMibOp() const noexcept
		{
			return hasFlag(kFlagMib);
		}
		//! Get if the instruction uses VSIB.
		ASMJIT_INLINE bool isVsibOp() const noexcept
		{
			return hasFlag(kFlagVsib);
		}
		//! Get if the instruction uses VEX (can be set together with EVEX if both are encodable).
		ASMJIT_INLINE bool isVex() const noexcept
		{
			return hasFlag(kFlagVex);
		}
		//! Get if the instruction uses EVEX (can be set together with VEX if both are encodable).
		ASMJIT_INLINE bool isEvex() const noexcept
		{
			return hasFlag(kFlagEvex);
		}
		//! Get if the instruction uses VEX and/or EVEX.
		ASMJIT_INLINE bool isVexOrEvex() const noexcept
		{
			return hasFlag(kFlagVex | kFlagEvex);
		}

		//! Get if the instruction supports AVX512 masking {k}.
		ASMJIT_INLINE bool hasAvx512K() const noexcept
		{
			return hasFlag(kFlagAvx512K);
		}
		//! Get if the instruction supports AVX512 zeroing {k}{z}.
		ASMJIT_INLINE bool hasAvx512Z() const noexcept
		{
			return hasFlag(kFlagAvx512Z);
		}
		//! Get if the instruction supports AVX512 embedded-rounding {er}.
		ASMJIT_INLINE bool hasAvx512ER() const noexcept
		{
			return hasFlag(kFlagAvx512ER);
		}
		//! Get if the instruction supports AVX512 suppress-all-exceptions {sae}.
		ASMJIT_INLINE bool hasAvx512SAE() const noexcept
		{
			return hasFlag(kFlagAvx512SAE);
		}
		//! Get if the instruction supports AVX512 broadcast (either 32-bit or 64-bit).
		ASMJIT_INLINE bool hasAvx512B() const noexcept
		{
			return hasFlag(kFlagAvx512B32 | kFlagAvx512B64);
		}
		//! Get if the instruction supports AVX512 broadcast (32-bit).
		ASMJIT_INLINE bool hasAvx512B32() const noexcept
		{
			return hasFlag(kFlagAvx512B32);
		}
		//! Get if the instruction supports AVX512 broadcast (64-bit).
		ASMJIT_INLINE bool hasAvx512B64() const noexcept
		{
			return hasFlag(kFlagAvx512B64);
		}

		//! Get if the instruction may or will jump (returns true also for calls and returns).
		ASMJIT_INLINE bool doesJump() const noexcept
		{
			return _jumpType != Inst::kJumpTypeNone;
		}
		...
		ASMJIT_INLINE bool hasAltOpCode() const noexcept
		{
			return _altOpCodeIndex != 0;
		}
		//! Get alternative opcode, see \ref OpCodeBits.
		ASMJIT_INLINE uint32_t getAltOpCode() const noexcept;

		ASMJIT_INLINE uint32_t getISignatureIndex() const noexcept
		{
			return _iSignatureIndex;
		}
		ASMJIT_INLINE uint32_t getISignatureCount() const noexcept
		{
			return _iSignatureCount;
		}
		...
		ASMJIT_INLINE uint32_t getJumpType() const noexcept
		{
			return _jumpType;
		}
		ASMJIT_INLINE uint32_t getSingleRegCase() const noexcept
		{
			return _singleRegCase;
		}
	...
	{
		ASMJIT_INLINE uint32_t getOperationFlags() const noexcept
		{
			return _flags;
		}
		ASMJIT_INLINE bool hasOperationFlag(uint32_t flag) const noexcept
		{
			return (_flags & flag) != 0;
		}

		ASMJIT_INLINE bool isMovCrDr() const noexcept
		{
			return hasOperationFlag(kOperationMovCrDr);
		}
		ASMJIT_INLINE bool isMovSsSd() const noexcept
		{
			return hasOperationFlag(kOperationMovSsSd);
		}

		ASMJIT_INLINE bool isPrefetch() const noexcept
		{
			return hasOperationFlag(kOperationPrefetch);
		}
		ASMJIT_INLINE bool isBarrier() const noexcept
		{
			return hasOperationFlag(kOperationBarrier);
		}
		ASMJIT_INLINE bool isVolatile() const noexcept
		{
			return hasOperationFlag(kOperationVolatile);
		}
		ASMJIT_INLINE bool isPrivileged() const noexcept
		{
			return hasOperationFlag(kOperationPrivileged);
		}
		...
		ASMJIT_INLINE uint32_t getSpecialRegsR() const noexcept
		{
			return _specialRegsR;
		}
		ASMJIT_INLINE uint32_t getSpecialRegsW() const noexcept
		{
			return _specialRegsW;
		}

		ASMJIT_INLINE const uint8_t* getFeaturesData() const noexcept
		{
			return _features;
		}
		ASMJIT_INLINE const uint8_t* getFeaturesEnd() const noexcept
		{
			return _features + ASMJIT_ARRAY_SIZE(_features);
		}
	...
	struct SseToAvxData
	{
		ASMJIT_INLINE uint32_t getMode() const noexcept
		{
			return _mode;
		}
		ASMJIT_INLINE int32_t getDelta() const noexcept
		{
			return _delta;
		}
	...
	//! NOTE: If AsmJit was compiled with `ASMJIT_DISABLE_TEXT` then this will
	//! always return zero.
	ASMJIT_INLINE uint32_t getNameDataIndex() const noexcept
	{
		return _nameDataIndex;
	}
	...
	ASMJIT_INLINE uint32_t getCommonDataIndex() const noexcept
	{
		return _commonDataIndex;
	}
	...
	ASMJIT_INLINE uint32_t getOperationDataIndex() const noexcept
	{
		return _operationDataIndex;
	}
	...
	ASMJIT_INLINE uint32_t getSseToAvxDataIndex() const noexcept
	{
		return _sseToAvxDataIndex;
	}

	//! Get instruction encoding, see \ref EncodingType.
	ASMJIT_INLINE uint32_t getEncodingType() const noexcept
	{
		return _encodingType;
	}

	//! Get if the instruction has main opcode (rare, but it's possible it doesn't have).
	ASMJIT_INLINE bool hasMainOpCode() const noexcept
	{
		return _mainOpCode != 0;
	}
	//! Get main opcode, see \ref OpCodeBits.
	ASMJIT_INLINE uint32_t getMainOpCode() const noexcept
	{
		return _mainOpCode;
	}
	...
	ASMJIT_INLINE bool hasAltOpCode() const noexcept
	{
		return getCommonData().hasAltOpCode();
	}
	//! Get alternative opcode, see \ref OpCodeBits.
	ASMJIT_INLINE uint32_t getAltOpCode() const noexcept
	{
		return getCommonData().getAltOpCode();
	}

	//! Get if the instruction has flag `flag`, see \ref Flags.
	ASMJIT_INLINE bool hasFlag(uint32_t flag) const noexcept
	{
		return getCommonData().hasFlag(flag);
	}
	//! Get instruction flags, see \ref Flags.
	ASMJIT_INLINE uint32_t getFlags() const noexcept
	{
		return getCommonData().getFlags();
	}

	//! Get if the instruction is FPU instruction.
	ASMJIT_INLINE bool isFpu() const noexcept
	{
		return getCommonData().isFpu();
	}
	//! Get if the instruction is MMX instruction that accesses MMX registersm, including EMMS.
	ASMJIT_INLINE bool isMmx() const noexcept
	{
		return getCommonData().isMmx();
	}

	//! Get if the instruction is SSE|AVX|AVX512 instruction that accesses XMM|YMM|ZMM registers.
	ASMJIT_INLINE bool isVec() const noexcept
	{
		return getCommonData().isVec();
	}
	//! Get if the instruction is SSE+ (SSE4.2, AES, SHA included) instruction that accesses XMM registers.
	ASMJIT_INLINE bool isSse() const noexcept
	{
		return getCommonData().isSse();
	}
	//! Get if the instruction is AVX+ (FMA included) instruction that accesses XMM|YMM|ZMM registers.
	ASMJIT_INLINE bool isAvx() const noexcept
	{
		return getCommonData().isAvx();
	}

	//! Get if the instruction can be prefixed by LOCK prefix.
	ASMJIT_INLINE bool isLockEnabled() const noexcept
	{
		return getCommonData().isLockEnabled();
	}
	//! Get if the instruction can be prefixed by REP prefix.
	ASMJIT_INLINE bool isRepEnabled() const noexcept
	{
		return getCommonData().isRepEnabled();
	}
	//! Get if the instruction can be prefixed by REPZ prefix.
	ASMJIT_INLINE bool isRepzEnabled() const noexcept
	{
		return getCommonData().isRepzEnabled();
	}
	//! Get if the instruction can be prefixed by REPNZ prefix.
	ASMJIT_INLINE bool isRepnzEnabled() const noexcept
	{
		return getCommonData().isRepnzEnabled();
	}

	//! Get if the instruction uses MIB.
	ASMJIT_INLINE bool isMibOp() const noexcept
	{
		return getCommonData().isMibOp();
	}
	//! Get if the instruction uses VSIB.
	ASMJIT_INLINE bool isVsibOp() const noexcept
	{
		return getCommonData().isVsibOp();
	}
	//! Get if the instruction uses VEX (can be set together with EVEX if both are encodable).
	ASMJIT_INLINE bool isVex() const noexcept
	{
		return getCommonData().isVex();
	}
	//! Get if the instruction uses EVEX (can be set together with VEX if both are encodable).
	ASMJIT_INLINE bool isEvex() const noexcept
	{
		return getCommonData().isEvex();
	}

	//! Get if the instruction supports AVX512 masking {k}.
	ASMJIT_INLINE bool hasAvx512K() const noexcept
	{
		return getCommonData().hasAvx512K();
	}
	//! Get if the instruction supports AVX512 zeroing {k}{z}.
	ASMJIT_INLINE bool hasAvx512Z() const noexcept
	{
		return getCommonData().hasAvx512Z();
	}
	//! Get if the instruction supports AVX512 embedded-rounding {er}.
	ASMJIT_INLINE bool hasAvx512ER() const noexcept
	{
		return getCommonData().hasAvx512ER();
	}
	//! Get if the instruction supports AVX512 suppress-all-exceptions {sae}.
	ASMJIT_INLINE bool hasAvx512SAE() const noexcept
	{
		return getCommonData().hasAvx512SAE();
	}
	//! Get if the instruction supports AVX512 broadcast (either 32-bit or 64-bit).
	ASMJIT_INLINE bool hasAvx512B() const noexcept
	{
		return getCommonData().hasAvx512B();
	}
	//! Get if the instruction supports AVX512 broadcast (32-bit).
	ASMJIT_INLINE bool hasAvx512B32() const noexcept
	{
		return getCommonData().hasAvx512B32();
	}
	//! Get if the instruction supports AVX512 broadcast (64-bit).
	ASMJIT_INLINE bool hasAvx512B64() const noexcept
	{
		return getCommonData().hasAvx512B64();
	}

	ASMJIT_INLINE uint32_t getISignatureIndex() const noexcept
	{
		return getCommonData().getISignatureIndex();
	}
	ASMJIT_INLINE uint32_t getISignatureCount() const noexcept
	{
		return getCommonData().getISignatureCount();
	}

	ASMJIT_INLINE const ISignature* getISignatureData() const noexcept
	{
		return getCommonData().getISignatureData();
	}
	ASMJIT_INLINE const ISignature* getISignatureEnd() const noexcept
	{
		return getCommonData().getISignatureEnd();
	}
	...
	static ASMJIT_INLINE bool isDefinedId(uint32_t instId) noexcept
	{
...
ASMJIT_INLINE const X86Inst::OperationData& X86Inst::getOperationData() const noexcept
{
	return X86InstDB::operationData[_operationDataIndex];
}
ASMJIT_INLINE const X86Inst::SseToAvxData& X86Inst::getSseToAvxData() const noexcept
{
	return X86InstDB::sseToAvxData[_sseToAvxDataIndex];
}
