...
struct TiedReg
{
	...
	ASMJIT_INLINE uint32_t hasInPhysId() const
	{
		return inPhysId != Globals::kInvalidRegId;
	}
	//! Get whether the variable has to be allocated in a specific output register.
	ASMJIT_INLINE uint32_t hasOutPhysId() const
	{
		return outPhysId != Globals::kInvalidRegId;
	}

	//! Set the input register index.
	ASMJIT_INLINE void setInPhysId(uint32_t index)
	{
		inPhysId = static_cast<uint8_t>(index);
	}
	//! Set the output register index.
	ASMJIT_INLINE void setOutPhysId(uint32_t index)
	{
		outPhysId = static_cast<uint8_t>(index);
	}
...
public:
	...
	ASMJIT_INLINE CodeCompiler* cc() const noexcept
	{
		return static_cast<CodeCompiler*>(_cb);
	}
	...
	//! Get stop node.
	ASMJIT_INLINE CBNode* getStop() const noexcept
	{
		return _stop;
	}
	...
	//! Get current state.
	ASMJIT_INLINE RAState* getState() const
	{
		return _state;
	}
