...
public:
	...
	static ASMJIT_INLINE bool isX86Family(uint32_t archType) noexcept
	{
		return archType >= kTypeX86 && archType <= kTypeX32;
	}
	static ASMJIT_INLINE bool isArmFamily(uint32_t archType) noexcept
	{
		return archType >= kTypeA32 && archType <= kTypeA64;
	}
	...
	ASMJIT_INLINE static ArchInfo host() noexcept
	{
		return ArchInfo(kTypeHost, kSubTypeHost);
	}
	...
	//!
	//! No meaning yet.
	ASMJIT_INLINE uint32_t getSubType() const noexcept
	{
		return _subType;
	}

	//! Get if the architecture is X86, X64, or X32.
	ASMJIT_INLINE bool isX86Family() const noexcept
	{
		return isX86Family(_type);
	}
	//! Get if the architecture is ARM32 or ARM64.
	ASMJIT_INLINE bool isArmFamily() const noexcept
	{
		return isArmFamily(_type);
	}
	...
	ASMJIT_INLINE bool operator==(const ArchInfo& other) const noexcept
	{
