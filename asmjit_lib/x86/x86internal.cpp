        ...
        const Operand_& src_, uint32_t typeId, bool avxEnabled, const char* comment)
{
	...
	{
		memFlags |= kDstMem;
		dst.as<X86Mem>().setSize(src.getSize());
	}
	if (src.isMem())
	{
		memFlags |= kSrcMem;
		src.as<X86Mem>().setSize(dst.getSize());
	}
	...
	case TypeId::kMmx64 :
		...
		break;
	case TypeId::kMask8 :
		instId = X86Inst::kIdKmovb;
		break;
	case TypeId::kMask16:
		instId = X86Inst::kIdKmovw;
		break;
	case TypeId::kMask32:
		instId = X86Inst::kIdKmovd;
		break;
	case TypeId::kMask64:
		instId = X86Inst::kIdKmovq;
		break;
