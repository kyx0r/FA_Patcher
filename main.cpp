...
{	
	...
	cout<<"What to do? p - full patch, s - patch ignoring hooks, cdbg - convert x64dbg disassembly to gcc inline"<<endl;
	cin >> reply;
	if(reply.compare("p") == 0 || reply.compare("s") == 0)
	{
	...
	{
		reply.empty();
		cout<<"Enter filename: ( 'd' for default. [preprocessor/x64dbg.asm] )"<<endl;
		cin>>reply;
