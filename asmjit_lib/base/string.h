...
public:
	...
	ASMJIT_INLINE SmallString() noexcept
	{
		reset();
	}
	...
	ASMJIT_INLINE bool isEmbedded() const noexcept
	{
		return _length <= kMaxEmbeddedLength;
	}
	ASMJIT_INLINE bool mustEmbed(size_t len) const noexcept
	{
		return len <= kMaxEmbeddedLength;
	}

	ASMJIT_INLINE uint32_t getLength() const noexcept
	{
		return _length;
	}
...
public:
	...
	ASMJIT_INLINE char* getData() noexcept
	{
		return _data;
	}
	//! Get null-terminated string data (const).
	ASMJIT_INLINE const char* getData() const noexcept
	{
		return _data;
	}
	...
	ASMJIT_INLINE Error setString(const char* str, size_t len = Globals::kInvalidIndex) noexcept
	{
	...
	ASMJIT_INLINE Error appendString(const char* str, size_t len = Globals::kInvalidIndex) noexcept
	{
	...
	ASMJIT_INLINE Error appendFormatVA(const char* fmt, va_list ap) noexcept
	{
		return _opVFormat(kStringOpAppend, fmt, ap);
	}

	//! Append a single `c` character.
	ASMJIT_INLINE Error appendChar(char c) noexcept
	{
		return _opChar(kStringOpAppend, c);
	}
	//! Append `c` character `n` times.
	ASMJIT_INLINE Error appendChars(char c, size_t n) noexcept
	{
	...
	ASMJIT_INLINE bool eq(const StringBuilder& other) const noexcept
	{
		return eq(other._data, other._length);
	}
	...
	ASMJIT_INLINE bool operator==(const StringBuilder& other) const noexcept
	{
		return  eq(other);
	}
	ASMJIT_INLINE bool operator!=(const StringBuilder& other) const noexcept
	...
	ASMJIT_INLINE bool operator==(const char* str) const noexcept
	{
		return  eq(str);
	}
	ASMJIT_INLINE bool operator!=(const char* str) const noexcept
