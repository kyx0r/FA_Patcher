...
class value_holder:public value_holder_base<T>
{
	...
	T*       data()noexcept
	{
		return reinterpret_cast<T*>(&this->s);
	}
...
private:
	...
	void copy(Allocator& al,const T& x)
	{
	...
	void move_assign(T&& x)
	{
		move_assign(std::move(x),is_move_assignable{});
	}
	...
	{
		return equal(x,is_equality_comparable{});
	}
...
struct value_holder_allocator_adaptor:Allocator
{
	...
	template<
	    ...
	    typename std::enable_if<
	        ...
	        >::type* =nullptr
	    >
	value_holder_allocator_adaptor(const Allocator2& x)noexcept:Allocator
	{
		x
	} {}
	...
	value_holder_allocator_adaptor(
		...
		Allocator
	{
		static_cast<const Allocator2&>(x)
	} {}
