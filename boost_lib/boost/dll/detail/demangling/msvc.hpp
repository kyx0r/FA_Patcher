...
auto const_rule_impl(true_type )
{
	return x3::space >> "const";
};
auto const_rule_impl(false_type)
{
	return x3::eps;
};
template<typename T>
auto const_rule()
{
	using t = is_const<typename remove_reference<T>::type>;
	return const_rule_impl(t());
}

auto volatile_rule_impl(true_type )
{
	return x3::space >> "volatile";
};
auto volatile_rule_impl(false_type)
{
	return x3::eps;
};
template<typename T>
auto volatile_rule()
{
	using t = is_volatile<typename remove_reference<T>::type>;
	return volatile_rule_impl(t());
}


auto inv_const_rule_impl(true_type )
{
	return "const" >>  x3::space ;
};
auto inv_const_rule_impl(false_type)
{
	return x3::eps;
};
template<typename T>
...
{
	using t = is_const<typename remove_reference<T>::type>;
	return inv_const_rule_impl(t());
}

auto inv_volatile_rule_impl(true_type )
{
	return "volatile" >> x3::space;
};
auto inv_volatile_rule_impl(false_type)
{
	return x3::eps;
};
template<typename T>
auto inv_volatile_rule()
{
	using t = is_volatile<typename remove_reference<T>::type>;
	return inv_volatile_rule_impl(t());
}


auto reference_rule_impl(false_type, false_type)
{
	return x3::eps;
}
auto reference_rule_impl(true_type,  false_type)
{
	return x3::space >>"&"  ;
}
auto reference_rule_impl(false_type, true_type )
{
	return x3::space >>"&&" ;
}
...
auto reference_rule()
{
	using t_l = is_lvalue_reference<T>;
	using t_r = is_rvalue_reference<T>;
	return reference_rule_impl(t_l(), t_r());
}
...
template<>
auto type_rule<void>(const std::string &)
{
