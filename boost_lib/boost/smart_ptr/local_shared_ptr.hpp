...
BOOST_CONSTEXPR local_shared_ptr() BOOST_SP_NOEXCEPT :
	px( 0 ), pn( 0 )
	{
	}
...
BOOST_CONSTEXPR local_shared_ptr( boost::detail::sp_nullptr_t ) BOOST_SP_NOEXCEPT :
	px( 0 ), pn( 0 )
	{
	}
...
BOOST_CONSTEXPR local_shared_ptr( boost::detail::lsp_internal_constructor_tag, element_type * px_, boost::detail::local_counted_base * pn_ ) BOOST_SP_NOEXCEPT :
	px( px_ ), pn( pn_ )
	{
	}
...
local_shared_ptr( local_shared_ptr const & r ) BOOST_SP_NOEXCEPT :
	px( r.px ), pn( r.pn )
	{
		if( pn )
		{
...
local_shared_ptr( local_shared_ptr && r ) BOOST_SP_NOEXCEPT :
	px( r.px ), pn( r.pn )
	{
		r.px = 0;
		r.pn = 0;
...
:
	...
	template<class Y>
local_shared_ptr( local_shared_ptr<Y> const & r, element_type * p ) BOOST_SP_NOEXCEPT :
	px( p ), pn( r.pn )
	{
		if( pn )
		{
