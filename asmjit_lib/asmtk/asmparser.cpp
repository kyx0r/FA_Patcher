...
static bool x86ParseRegister(Operand_& op, const uint8_t* s, size_t len)
{
	...
	if (len == 3)
	{
		...
		if (c0 == 'e' || c0 == 'r')
		{
			...
			if (cn == COMB_CHAR_2('a', 'x'))
			{
				rId = X86Gp::kIdAx;
				goto Done;
			}
			if (cn == COMB_CHAR_2('d', 'x'))
			{
				rId = X86Gp::kIdDx;
				goto Done;
			}
			if (cn == COMB_CHAR_2('b', 'x'))
			{
				rId = X86Gp::kIdBx;
				goto Done;
			}
			if (cn == COMB_CHAR_2('c', 'x'))
			{
				rId = X86Gp::kIdCx;
				goto Done;
			}
...
TrySpBpSiDi:
			if (cn == COMB_CHAR_2('s', 'p'))
			{
				rId = X86Gp::kIdSp;
				goto Done;
			}
			if (cn == COMB_CHAR_2('b', 'p'))
			{
				rId = X86Gp::kIdBp;
				goto Done;
			}
			if (cn == COMB_CHAR_2('s', 'i'))
			{
				rId = X86Gp::kIdSi;
				goto Done;
			}
			if (cn == COMB_CHAR_2('d', 'i'))
			{
				rId = X86Gp::kIdDi;
				goto Done;
			}
		...
		if (isStr)
		{
			if (opCount == 0)
			{
				...
				case X86Inst::kIdCmps:
					opArray[0] = emitter->ptr_zsi();
					opArray[1] = emitter->ptr_zdi();
					break;
				case X86Inst::kIdMovs:
					opArray[0] = emitter->ptr_zdi();
					opArray[1] = emitter->ptr_zsi();
					break;
				case X86Inst::kIdLods:
				case X86Inst::kIdScas:
					opArray[0] = Reg::fromSignature(sign, X86Gp::kIdAx);
					opArray[1] = emitter->ptr_zdi();
					break;
				case X86Inst::kIdStos:
					opArray[0] = emitter->ptr_zdi();
					opArray[1] = Reg::fromSignature(sign, X86Gp::kIdAx);
					break;
				}
			}
