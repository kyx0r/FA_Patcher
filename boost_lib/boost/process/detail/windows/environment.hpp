...
class native_environment_impl
{
	...
	{
		boost::winapi::free_environment_strings(p);
	};
...
public:
	...
	void        set(const pointer_type id, const pointer_type value)
	{
		set(string_type(id), value);
	}
	void      reset(const pointer_type id)
	{
		reset(string_type(id));
	}
...
template<> constexpr  char   env_seperator()
{
	return  ';';
}
template<> constexpr wchar_t env_seperator()
{
	return L';';
}
