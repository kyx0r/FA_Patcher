...
struct X86EmitterExplicitT
{
	...
	ASMJIT_INLINE const X86Gp& zax() const noexcept
	{
		return gpzRef(X86Gp::kIdAx);
	}
	ASMJIT_INLINE const X86Gp& zcx() const noexcept
	{
		return gpzRef(X86Gp::kIdCx);
	}
	ASMJIT_INLINE const X86Gp& zdx() const noexcept
	{
		return gpzRef(X86Gp::kIdDx);
	}
	ASMJIT_INLINE const X86Gp& zbx() const noexcept
	{
		return gpzRef(X86Gp::kIdBx);
	}
	ASMJIT_INLINE const X86Gp& zsp() const noexcept
	{
		return gpzRef(X86Gp::kIdSp);
	}
	ASMJIT_INLINE const X86Gp& zbp() const noexcept
	{
		return gpzRef(X86Gp::kIdBp);
	}
	ASMJIT_INLINE const X86Gp& zsi() const noexcept
	{
		return gpzRef(X86Gp::kIdSi);
	}
	ASMJIT_INLINE const X86Gp& zdi() const noexcept
	{
		return gpzRef(X86Gp::kIdDi);
	}
	...
	ASMJIT_INLINE X86Mem ptr_zax(int32_t off = 0, uint32_t size = 0) const noexcept
	{
		return ptr_base(X86Gp::kIdAx, off, size);
	}
	ASMJIT_INLINE X86Mem ptr_zcx(int32_t off = 0, uint32_t size = 0) const noexcept
	{
		return ptr_base(X86Gp::kIdCx, off, size);
	}
	ASMJIT_INLINE X86Mem ptr_zdx(int32_t off = 0, uint32_t size = 0) const noexcept
	{
		return ptr_base(X86Gp::kIdDx, off, size);
	}
	ASMJIT_INLINE X86Mem ptr_zbx(int32_t off = 0, uint32_t size = 0) const noexcept
	{
		return ptr_base(X86Gp::kIdBx, off, size);
	}
	ASMJIT_INLINE X86Mem ptr_zsp(int32_t off = 0, uint32_t size = 0) const noexcept
	{
		return ptr_base(X86Gp::kIdSp, off, size);
	}
	ASMJIT_INLINE X86Mem ptr_zbp(int32_t off = 0, uint32_t size = 0) const noexcept
	{
		return ptr_base(X86Gp::kIdBp, off, size);
	}
	ASMJIT_INLINE X86Mem ptr_zsi(int32_t off = 0, uint32_t size = 0) const noexcept
	{
		return ptr_base(X86Gp::kIdSi, off, size);
	}
	ASMJIT_INLINE X86Mem ptr_zdi(int32_t off = 0, uint32_t size = 0) const noexcept
	{
		return ptr_base(X86Gp::kIdDi, off, size);
	}
	...
	ASMJIT_INLINE Error db(uint8_t x)
	{
		return static_cast<This*>(this)->embed(&x, 1);
	}
	//! Add 16-bit integer data to the instruction stream.
	ASMJIT_INLINE Error dw(uint16_t x)
	{
		return static_cast<This*>(this)->embed(&x, 2);
	}
	//! Add 32-bit integer data to the instruction stream.
	ASMJIT_INLINE Error dd(uint32_t x)
	{
		return static_cast<This*>(this)->embed(&x, 4);
	}
	//! Add 64-bit integer data to the instruction stream.
	ASMJIT_INLINE Error dq(uint64_t x)
	{
		return static_cast<This*>(this)->embed(&x, 8);
	}

	//! Add 8-bit integer data to the instruction stream.
	ASMJIT_INLINE Error dint8(int8_t x)
	{
		return static_cast<This*>(this)->embed(&x, sizeof(int8_t));
	}
	//! Add 8-bit integer data to the instruction stream.
	ASMJIT_INLINE Error duint8(uint8_t x)
	{
		return static_cast<This*>(this)->embed(&x, sizeof(uint8_t));
	}

	//! Add 16-bit integer data to the instruction stream.
	ASMJIT_INLINE Error dint16(int16_t x)
	{
		return static_cast<This*>(this)->embed(&x, sizeof(int16_t));
	}
	//! Add 16-bit integer data to the instruction stream.
	ASMJIT_INLINE Error duint16(uint16_t x)
	{
		return static_cast<This*>(this)->embed(&x, sizeof(uint16_t));
	}

	//! Add 32-bit integer data to the instruction stream.
	ASMJIT_INLINE Error dint32(int32_t x)
	{
		return static_cast<This*>(this)->embed(&x, sizeof(int32_t));
	}
	//! Add 32-bit integer data to the instruction stream.
	ASMJIT_INLINE Error duint32(uint32_t x)
	{
		return static_cast<This*>(this)->embed(&x, sizeof(uint32_t));
	}

	//! Add 64-bit integer data to the instruction stream.
	ASMJIT_INLINE Error dint64(int64_t x)
	{
		return static_cast<This*>(this)->embed(&x, sizeof(int64_t));
	}
	//! Add 64-bit integer data to the instruction stream.
	ASMJIT_INLINE Error duint64(uint64_t x)
	{
		return static_cast<This*>(this)->embed(&x, sizeof(uint64_t));
	}

	//! Add float data to the instruction stream.
	ASMJIT_INLINE Error dfloat(float x)
	{
		return static_cast<This*>(this)->embed(&x, sizeof(float));
	}
	//! Add double data to the instruction stream.
	ASMJIT_INLINE Error ddouble(double x)
	{
		return static_cast<This*>(this)->embed(&x, sizeof(double));
	}

	//! Add MMX data to the instruction stream.
	ASMJIT_INLINE Error dmm(const Data64& x)
	{
		return static_cast<This*>(this)->embed(&x, sizeof(Data64));
	}
	//! Add XMM data to the instruction stream.
	ASMJIT_INLINE Error dxmm(const Data128& x)
	{
		return static_cast<This*>(this)->embed(&x, sizeof(Data128));
	}
	//! Add YMM data to the instruction stream.
	ASMJIT_INLINE Error dymm(const Data256& x)
	{
		return static_cast<This*>(this)->embed(&x, sizeof(Data256));
	}

	//! Add data in a given structure instance to the instruction stream.
	template<typename T>
	ASMJIT_INLINE Error dstruct(const T& x)
	{
		return static_cast<This*>(this)->embed(&x, static_cast<uint32_t>(sizeof(T)));
	}
...
public:
	...
	ASMJIT_INLINE This& short_() noexcept
	{
		return _addOptions(X86Inst::kOptionShortForm);
	}
	//! Force long form of jmp/jcc instruction.
	ASMJIT_INLINE This& long_() noexcept
	{
		return _addOptions(X86Inst::kOptionLongForm);
	}

	//! Condition is likely to be taken (has only benefit on P4).
	ASMJIT_INLINE This& taken() noexcept
	{
		return _addOptions(X86Inst::kOptionTaken);
	}
	//! Condition is unlikely to be taken (has only benefit on P4).
	ASMJIT_INLINE This& notTaken() noexcept
	{
		return _addOptions(X86Inst::kOptionNotTaken);
	}

	//! Use LOCK prefix.
	ASMJIT_INLINE This& lock() noexcept
	{
		return _addOptions(X86Inst::kOptionLock);
	}
	...
	//! Use REP/REPZ prefix.
	ASMJIT_INLINE This& repe(const X86Gp& zcx) noexcept
	{
		return rep(zcx);
	}
	//! Use REP/REPZ prefix.
	ASMJIT_INLINE This& repz(const X86Gp& zcx) noexcept
	{
		return rep(zcx);
	}
	//! Use REPNZ prefix.
	ASMJIT_INLINE This& repne(const X86Gp& zcx) noexcept
	{
		return repnz(zcx);
	}
	...
	ASMJIT_INLINE This& mod_mr() noexcept
	{
		return _addOptions(X86Inst::kOptionModMR);
	}
	...
	ASMJIT_INLINE This& rex() noexcept
	{
		return _addOptions(X86Inst::kOptionRex);
	}

	//! Force REX.B prefix (X64) [It exists for special purposes only].
	ASMJIT_INLINE This& rex_b() noexcept
	{
		return _addOptions(X86Inst::kOptionOpCodeB);
	}
	//! Force REX.X prefix (X64) [It exists for special purposes only].
	ASMJIT_INLINE This& rex_x() noexcept
	{
		return _addOptions(X86Inst::kOptionOpCodeX);
	}
	//! Force REX.R prefix (X64) [It exists for special purposes only].
	ASMJIT_INLINE This& rex_r() noexcept
	{
		return _addOptions(X86Inst::kOptionOpCodeR);
	}
	//! Force REX.W prefix (X64) [It exists for special purposes only].
	ASMJIT_INLINE This& rex_w() noexcept
	{
		return _addOptions(X86Inst::kOptionOpCodeW);
	}

	//! Force 3-byte VEX prefix (AVX+).
	ASMJIT_INLINE This& vex3() noexcept
	{
		return _addOptions(X86Inst::kOptionVex3);
	}
	//! Force 4-byte EVEX prefix (AVX512+).
	ASMJIT_INLINE This& evex() noexcept
	{
		return _addOptions(X86Inst::kOptionEvex);
	}

	//! Use zeroing instead of merging (AVX512+).
	ASMJIT_INLINE This& z() noexcept
	{
		return _addOptions(X86Inst::kOptionZMask);
	}
	//! Broadcast one element to all other elements (AVX512+).
	ASMJIT_INLINE This& _1tox() noexcept
	{
		return _addOptions(X86Inst::kOption1ToX);
	}

	//! Suppress all exceptions (AVX512+).
	ASMJIT_INLINE This& sae() noexcept
	{
		return _addOptions(X86Inst::kOptionSAE);
	}
	//! Static rounding mode {rn} (round-to-nearest even) and {sae} (AVX512+).
	ASMJIT_INLINE This& rn_sae() noexcept
	{
		return _addOptions(X86Inst::kOptionER | X86Inst::kOptionRN_SAE);
	}
	//! Static rounding mode {rd} (round-down, toward -inf) and {sae} (AVX512+).
	ASMJIT_INLINE This& rd_sae() noexcept
	{
		return _addOptions(X86Inst::kOptionER | X86Inst::kOptionRD_SAE);
	}
	//! Static rounding mode {ru} (round-up, toward +inf) and {sae} (AVX512+).
	ASMJIT_INLINE This& ru_sae() noexcept
	{
		return _addOptions(X86Inst::kOptionER | X86Inst::kOptionRU_SAE);
	}
	//! Static rounding mode {rz} (round-toward-zero, truncate) and {sae} (AVX512+).
	ASMJIT_INLINE This& rz_sae() noexcept
	{
		return _addOptions(X86Inst::kOptionER | X86Inst::kOptionRZ_SAE);
	}
...
struct X86EmitterImplicitT : public X86EmitterExplicitT<This>
{
	...
	//! Use REP/REPZ prefix.
	ASMJIT_INLINE This& rep() noexcept
	{
		return X86EmitterExplicitT<This>::_addOptions(X86Inst::kOptionRep);
	}
	//! Use REPNZ prefix.
	ASMJIT_INLINE This& repnz() noexcept
	{
		return X86EmitterExplicitT<This>::_addOptions(X86Inst::kOptionRepnz);
	}

	//! Use REP/REPZ prefix.
	ASMJIT_INLINE This& repe() noexcept
	{
		return rep();
	}
	//! Use REP/REPZ prefix.
	ASMJIT_INLINE This& repz() noexcept
	{
		return rep();
	}
	//! Use REPNZ prefix.
	ASMJIT_INLINE This& repne() noexcept
	{
		return repnz();
	}
	...
	ASMJIT_INLINE Error cmpsb()
	{
		return ASMJIT_EMIT(X86Inst::kIdCmps, X86EmitterExplicitT<This>::ptr_zsi(0, 1), X86EmitterExplicitT<This>::ptr_zdi(0, 1));
	}
	ASMJIT_INLINE Error cmpsd()
	{
		return ASMJIT_EMIT(X86Inst::kIdCmps, X86EmitterExplicitT<This>::ptr_zsi(0, 4), X86EmitterExplicitT<This>::ptr_zdi(0, 4));
	}
	ASMJIT_INLINE Error cmpsq()
	{
		return ASMJIT_EMIT(X86Inst::kIdCmps, X86EmitterExplicitT<This>::ptr_zsi(0, 8), X86EmitterExplicitT<This>::ptr_zdi(0, 8));
	}
	ASMJIT_INLINE Error cmpsw()
	{
		return ASMJIT_EMIT(X86Inst::kIdCmps, X86EmitterExplicitT<This>::ptr_zsi(0, 2), X86EmitterExplicitT<This>::ptr_zdi(0, 2));
	}

	ASMJIT_INLINE Error lodsb()
	{
		return ASMJIT_EMIT(X86Inst::kIdLods, x86::al, X86EmitterExplicitT<This>::ptr_zdi(0, 1));
	}
	ASMJIT_INLINE Error lodsd()
	{
		return ASMJIT_EMIT(X86Inst::kIdLods, x86::eax, X86EmitterExplicitT<This>::ptr_zdi(0, 4));
	}
	ASMJIT_INLINE Error lodsq()
	{
		return ASMJIT_EMIT(X86Inst::kIdLods, x86::rax, X86EmitterExplicitT<This>::ptr_zdi(0, 8));
	}
	ASMJIT_INLINE Error lodsw()
	{
		return ASMJIT_EMIT(X86Inst::kIdLods, x86::ax, X86EmitterExplicitT<This>::ptr_zdi(0, 2));
	}

	ASMJIT_INLINE Error movsb()
	{
		return ASMJIT_EMIT(X86Inst::kIdMovs, X86EmitterExplicitT<This>::ptr_zdi(0, 1), X86EmitterExplicitT<This>::ptr_zsi(0, 1));
	}
	ASMJIT_INLINE Error movsd()
	{
		return ASMJIT_EMIT(X86Inst::kIdMovs, X86EmitterExplicitT<This>::ptr_zdi(0, 4), X86EmitterExplicitT<This>::ptr_zsi(0, 4));
	}
	ASMJIT_INLINE Error movsq()
	{
		return ASMJIT_EMIT(X86Inst::kIdMovs, X86EmitterExplicitT<This>::ptr_zdi(0, 8), X86EmitterExplicitT<This>::ptr_zsi(0, 8));
	}
	ASMJIT_INLINE Error movsw()
	{
		return ASMJIT_EMIT(X86Inst::kIdMovs, X86EmitterExplicitT<This>::ptr_zdi(0, 2), X86EmitterExplicitT<This>::ptr_zsi(0, 2));
	}

	ASMJIT_INLINE Error scasb()
	{
		return ASMJIT_EMIT(X86Inst::kIdScas, x86::al, X86EmitterExplicitT<This>::ptr_zdi(0, 1));
	}
	ASMJIT_INLINE Error scasd()
	{
		return ASMJIT_EMIT(X86Inst::kIdScas, x86::eax, X86EmitterExplicitT<This>::ptr_zdi(0, 4));
	}
	ASMJIT_INLINE Error scasq()
	{
		return ASMJIT_EMIT(X86Inst::kIdScas, x86::rax, X86EmitterExplicitT<This>::ptr_zdi(0, 8));
	}
	ASMJIT_INLINE Error scasw()
	{
		return ASMJIT_EMIT(X86Inst::kIdScas, x86::ax, X86EmitterExplicitT<This>::ptr_zdi(0, 2));
	}

	ASMJIT_INLINE Error stosb()
	{
		return ASMJIT_EMIT(X86Inst::kIdStos, X86EmitterExplicitT<This>::ptr_zdi(0, 1), x86::al );
	}
	ASMJIT_INLINE Error stosd()
	{
		return ASMJIT_EMIT(X86Inst::kIdStos, X86EmitterExplicitT<This>::ptr_zdi(0, 4), x86::eax);
	}
	ASMJIT_INLINE Error stosq()
	{
		return ASMJIT_EMIT(X86Inst::kIdStos, X86EmitterExplicitT<This>::ptr_zdi(0, 8), x86::rax);
	}
	ASMJIT_INLINE Error stosw()
	{
		return ASMJIT_EMIT(X86Inst::kIdStos, X86EmitterExplicitT<This>::ptr_zdi(0, 2), x86::ax );
	}
