...
struct bit_aligned_pixel_reference
{
	...
	template <typename P> const bit_aligned_pixel_reference& operator=(const P& p) const
	{
...
private:
	...
	template <typename Channel> void assign(const Channel& chan, mpl::false_) const
	{
		check_gray();
		gil::at_c<0>(*this)=chan;
	}
	template <typename Channel> bool equal (const Channel& chan, mpl::false_) const
	{
		check_gray();
