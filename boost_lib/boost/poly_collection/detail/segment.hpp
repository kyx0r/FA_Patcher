...
public:
	...
	segment(const segment& x):pimpl{x.impl().copy()}
	{
		set_sentinel();
	}
...
private:
	...
	segment(from_prototype,const segment& x):
		pimpl{x.impl().empty_copy()}
	{
		set_sentinel();
	}

	segment_backend&       impl()noexcept
	{
		return *pimpl;
	}
